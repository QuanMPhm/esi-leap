...
class DummyIDP(baseIDP.BaseIDP):
    ...
    def add_project(self, id, name, parent_id):
        self.dummy_project_list.append(
            Project(ProjectManager, {"id": id, "name": name, "parent_id": parent_id})
        )

    def add_user(self, id, name, project_id):
        self.dummy_user_list.append(
            User(UserManager, {"id": id, "name": name, "project_id": project_id})
        )
